...
def collect_data(queue_in):
    ...
    for message in consumer:
        ...
        for elem in transformed:
            elem_np = np.array(elem, dtype = np.float64)
            queue_in.put(elem_np)
...
def update_ui(queue_out):
    while True:
        result = queue_out.get()  # Queue から結果を取得
...
if __name__ == '__main__':
    ...
    consumer.start()
    producer.start()
    threading.Thread(target=update_ui, args=(queue_out,), daemon=True).start()
